...
const Body = () => {
  ...
  const getpassfromlocal = async () => {
    // localStorage.removeItem('Passwords')
    const jsondata = localStorage.getItem('Passwords')
    const passwords = jsondata ? await JSON.parse(jsondata) : []
  ...
  useEffect(() => {
    const json = JSON.stringify(pasarr)
    localStorage.setItem('Passwords', json)
  }, [pasarr])
  ...
  const handleSave = async () => {
    ...
    setpassarr([...pasarr, {id: crypto.randomUUID(), url: url, username: user, password: pass}])
